    ...
    <div className=" p-[10px]">
        <div className="w-full lg:w-max lg:h-[960px] flex lg:flex-col justify-between relative items-center bg-[#161D2F] p-4 rounded-[15px]">
            <div className="w-[100%] lg:w-max flex lg:flex-col items-center gap-0 lg:gap-[40px] ">
                ...
                <Link to={'/movies'} onClick={() => handleActive(2)}>
                <svg className={`${active === 2 ? 'fill-white' : 'fill-[#5a698f]' } `} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <Link to={'/tv'} onClick={() => handleActive(3)}>
                <svg className={`${active === 3 ? 'fill-white' : 'fill-[#5a698f]'  } `} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <Link to={'/bookmarked'} onClick={() => handleActive(4)}>
                <svg className={`${active === 4 ? 'fill-[#fff]' : 'fill-[#5a698f]' } `} width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {
                isOpen ? <div className="w-[150px] bg-red-600 p-[10px] flex justify-center rounded-[10px] absolute z-10 bottom-3 left-[80px]">
                <button onClick={() => removeStorage()} className="bg-[white] cursor-pointer grow text-black rounded-[20px]">Log out</button>
            </div> : null
            }
        </div>
    </div>
