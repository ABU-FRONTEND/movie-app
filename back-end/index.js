...
const connectUrl = process.env.MONGO_URL || "mongodb+srv://shojik:3NAqlYto5jmCQcTj@backend.6l8ntax.mongodb.net/?retryWrites=true&w=majority&appName=backend";
...
app.get('/trending', (req, res) => {
    res.status(200).send(data.filter((item) => item.isTrending === true));
});

app.get('/bookmarked', (req, res) => {
    res.status(200).send(data.filter((item) => item.isBookmarked === true));
});

app.get('/category/:category', (req, res) => {
    const category = req.params.category.toLowerCase();
    res.status(200).send(data.filter((item) => item.category.toLowerCase() === category));
});
